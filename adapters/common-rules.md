...
**执行策略:**
- **单步模式**(默认):执行 → 更新 → 刷新 → 下一个(适用:高风险、跨模块)
- **批量模式**(优化):执行2-3个 → 批量更新 → 刷新(适用:同模块、批量操作)
...
### 结构模板
- `{{TEMPLATE_DIR}}/project-mature.md` - project.md 模板
- `{{TEMPLATE_DIR}}/request.md` - 包含澄清对话和自检
- `{{TEMPLATE_DIR}}/todolist.md` - 精简版,仅任务清单
- `{{TEMPLATE_DIR}}/task_log.md` - 单个任务的详细日志模板

### Few-Shot 示例
- `{{EXAMPLE_DIR}}/01-new-feature/` - 新功能开发
- `{{EXAMPLE_DIR}}/02-refactor/` - 重构
- `{{EXAMPLE_DIR}}/03-debug/` - 调试

### 辅助工具
- `{{SCRIPT_DIR}}/refresh-context.sh` - 上下文刷新
- `{{SCRIPT_DIR}}/archive-task.sh` - 任务归档

### 最佳实践指南
建议在以下情况阅读 `{{DOCS_DIR}}/best-practices.md`:
- 接手新项目(冷启动流程)
- 不确定如何拆分文档
- 陷入循环或死胡同
- 了解高级技巧(如错误恢复模式)
